...
export class ApiFeature<T extends ObjectLiteral> {
  ...
  public filter(): ApiFeature<T> {
    ...
    Object.keys(queryObj).forEach((key) => {
      ...
      if (typeof value === 'string' && key.includes('[')) {
        ...
        if (key.endsWith('[gte]'))
          this.queryBuilder.andWhere(`${realkey} >= :${realkey}GTE`, {
            [`${realkey}GTE`]: value,
          });
        else if (key.endsWith('[gt]'))
          this.queryBuilder.andWhere(`${realkey} > :${realkey}GT`, {
            [`${realkey}GT`]: value,
          });
        else if (key.endsWith('[lte]'))
          this.queryBuilder.andWhere(`${realkey} <= :${realkey}LTE`, {
            [`${realkey}LTE`]: value,
          });
        else if (key.endsWith('[lt]'))
          this.queryBuilder.andWhere(`${realkey} < :${realkey}LT`, {
            [`${realkey}LT`]: value,
          });
        else if (key.endsWith('[ne]'))
          this.queryBuilder.andWhere(`${realkey} != :${realkey}NE`, {
            [`${realkey}NE`]: value,
          });
        else if (key.endsWith('[eq]'))
          this.queryBuilder.andWhere(`${realkey} = :${realkey}EQ`, {
            [`${realkey}EQ`]: value,
          });
      } else {
        if (key === 'tittle') {
          key = 'name';
          this.queryBuilder.andWhere(`${key} LIKE :${key}`, {
            [key]: `%${value}%`,
          });
    ...
    if (minPrice !== null && maxPrice !== null) {
      this.queryBuilder.andWhere(`price BETWEEN :minPrice AND :maxPrice`, {
        minPrice,
        maxPrice,
      });
    }
  ...
  public sort(): ApiFeature<T> {
    console.log(
      'Sorting by 1 ',
      typeof this.query.sort,
      this.query.sort.length,
    );
    if (this.query.sort) {
      ...
      this.queryBuilder.addOrderBy(
        `${this.queryBuilder.alias}.createdAt`,
        'DESC',
      );
    }
    return this;
